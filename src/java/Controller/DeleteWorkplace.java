...
/**
 * Interaction logic for DeleteWorkplace.
 */
public class DeleteWorkplace extends HttpServlet
{
    ...
    {
        Model.Relations relations = businessLogicLayer.ListRelations();
        Model.Relation relation = new Model.Relation();
        for (Model.Relation oneItem : relations.getItems())
        {
            if (oneItem.getRelationWorkplace() == Integer.parseInt(request.getParameter("id")) && relation.getRelationId() < 1)
            {
                relation = oneItem;
            }
        }
        ...
        if (request.getParameter("DeleteWorkplaceDeleteButton") != null)
        {
            ...
            {
                RequestDispatcher redirect = request.getRequestDispatcher("/WEB-INF/DeleteWorkplace.jsp");
